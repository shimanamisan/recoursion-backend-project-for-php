 ...
 * 文字列を置き換えるクラス
 */
class ReplaceString implements IFileOperation
{
    ...
    */
    public function __construct($inputFile, $newstring)
    {
    ...
    public function execute(): void
    {
        try {
            ...
            // ファイルの内容読み込み後はリソースを開放
            $this->fileClose();

            $pattern = '/needle/';

            // preg_match_allはマッチしたすべての部分を返す
            if (!preg_match_all($pattern, $readContents, $matches)) {
                throw new \Exception("needleという文字列は見つかりませんでした\n");
            }

            // preg_replaceはマッチしたすべての部分を置き換える
            $replacedContents = preg_replace($pattern, "newstring", $readContents);

            // 書き込み専用で開く
            $this->handle = fopen($this->inputFile, "w");

            if(!$this->handle) {
                throw new \Exception("ファイルの読み取りに失敗しました\n");
            }

            fwrite($this->handle, "{$replacedContents}");
